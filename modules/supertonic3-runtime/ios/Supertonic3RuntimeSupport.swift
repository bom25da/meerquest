...
private let supportedSupertonic3Voices: Set<String> = [
  "F1",
  "F2",
  "F3",
  "F4",
...
final class Supertonic3RuntimeService {
  ...
  func synthesize(text: String, options: [String: Any]) throws -> [String: Any] {
    ...
    let voice = options["voice"] as? String ?? "F2"
    ...
    guard FileManager.default.fileExists(atPath: voiceURL.path) else {
      throw Supertonic3RuntimeError("Supertonic 3 voice style is missing: \(voice).")
    }
    ...
    let outputURL = FileManager.default.temporaryDirectory
      .appendingPathComponent("supertonic3-\(UUID().uuidString).wav")
...
func manifestFiles(from manifest: [String: Any]) throws -> [Supertonic3ManifestFile] {
  ...
  return try files.map { item in
    ...
    return Supertonic3ManifestFile(
      path: path,
...
private func manifestByteCount(from value: Any?) throws -> Int {
  ...
  throw Supertonic3RuntimeError("Supertonic 3 manifest file byte count is malformed.")
}

private func validateSynthesisOptions(lang: String, voice: String, speed: Double, steps: Int) throws {
  guard isValidLang(lang) else {
    throw Supertonic3RuntimeError("Unsupported Supertonic 3 language: \(lang).")
  }

  guard supportedSupertonic3Voices.contains(voice) else {
    throw Supertonic3RuntimeError("Unsupported Supertonic 3 voice: \(voice).")
  }

  guard speed.isFinite, speed > 0, speed <= 4.0 else {
    throw Supertonic3RuntimeError("Supertonic 3 speed must be finite and greater than 0.")
  }

  guard steps > 0, steps <= 100 else {
    throw Supertonic3RuntimeError("Supertonic 3 steps must be greater than 0.")
  }
}
...
private func numericOption(_ value: Any?, name: String, defaultValue: Double) throws -> Double {
  ...
  throw Supertonic3RuntimeError("Supertonic 3 \(name) option must be numeric.")
}
...
private func intOption(_ value: Any?, name: String, defaultValue: Int) throws -> Int {
  ...
  if let value = value as? NSNumber {
    ...
    guard doubleValue.isFinite, doubleValue.rounded(.towardZero) == doubleValue else {
      throw Supertonic3RuntimeError("Supertonic 3 \(name) option must be an integer.")
    }
